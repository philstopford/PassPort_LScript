...
platformcheck
{
	// Check to block operation on older LW versions - permitted removal of needHackySaving and also the '::' variable prefix.
	if (hostBuild() < 2716)
	{
	  supportedplatform = 0;
	  logger("error","This version of PassPort requires at least LightWave 3D 11.6 build 2716.");
	}

	// Discovery Mode removes the ability to save scenes, objects, etc.
	// These limitations impact a lot of PassPort functionality, so warn the user.
	if(lwlicensecheck_done == 0)
	{
	  if(runningUnder() == LAYOUT && licenseId() == 0)
	  {
		logger("info","PassPort will not function properly due to Layout limitations in Discovery Mode");
	  }
	  lwlicensecheck_done = 1;
	}
...
load: what,io
{
	...
	if(what == SCENEMODE)
	{
		...
		{
			logger("warn","WARNING: This scene contains settings for Passport version " + passPortVersion.asStr());
			versionMismatch = 1;
		}
		lwVersion = io.read();
		if(lwVersion != hostVersion().asStr())
		{
			logger("warn","WARNING: Passport override settings are for LW version " + lwVersion.asStr());
			versionMismatch = 1;
		}
		...
		overrideNamesSize = int(io.read());
		overrideIndex = 1;
		while (overrideIndex <= overrideNamesSize)
		{
			::overrideNames[overrideIndex] = io.read();
			if (::overrideNames[overrideIndex] == "empty")
			{
				::overrideSettings[overrideIndex] = "";
			} else {
				::overrideSettings[overrideIndex] = io.read();
			}
			overrideIndex++;
		}
		for(x = 1; x <= passNamesSize; x++)
...
// custom interface functions below here
	preProcess
	{
		...
		::errorOnDependency = integer(globalrecall("passEditorerrorOnDependency", 1));
